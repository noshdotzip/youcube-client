...
-- OpenPastebinCleanup v1.0.0 (based on wget)

local url_to_cleanup = "https://raw.githubusercontent.com/noshdotzip/youcube-client/main/installer/src/cleanup.lua"

if not http then
    printError("OpenPastebinCleanup requires the http API")
...
local function http_get(url)
    ...
    printColoured(('Running "%s".'):format(getFilename(url)), colors.lime)
...
local response_body = http_get(url_to_cleanup)

local loaded_function, load_error_message = load(response_body, getFilename(url_to_cleanup), "t", _ENV)
